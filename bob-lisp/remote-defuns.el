...
(defun my-find-over-proxy (server-name)
  "Find file through PROXY on SERVER-NAME."
  (interactive "sEnter server name: ")
  (find-file (format-default-dir server-name "ubuntu" "shadow" nil)))
...
(defun run-shell-over-server (sudo server-name)
  (interactive "P 
sEnter server-name:\s")
  (let ((default-directory (format-default-dir server-name "ubuntu" "shadow" sudo))
        (shell-file-name "/bin/bash"))
    (save-excursion)
    (set-buffer (get-buffer-create (generate-buffer-name-for-server server-name)))
    (shell (generate-buffer-name-for-server server-name))))

(defun bob/shell (&optional user-shell-name)
  (interactive)
  (if-let* ((tramp-path (when (tramp-tramp-file-p default-directory)
                          (tramp-dissect-file-name default-directory)))
            (tramp-current-host (tramp-file-name-host tramp-path))
            (remote-shell-name (format "*SHELL*::%s" (upcase (or tramp-current-host "ENDER")))))
      (let ((shell-file-name "/bin/bash"))
       (shell remote-shell-name))
    (let ((shell-file-name "/bin/zsh"))
      (shell (format "*SHELL*::%s" (buffer-name))))))
