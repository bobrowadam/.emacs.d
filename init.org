...
#+begin_src elisp
  (use-package mentat
    :load-path "~/source/mentat/.worktrees/use-vui-instead-of-org/"
    :after (org)
    ;; :ensure (mentat
    ...
    (mentat-server-path "~/source/mentat/.worktrees/use-vui-instead-of-org/mentat-server/")
...
#+begin_src elisp
  ...
  (use-package ob-bun
    :ensure nil
    :load-path "~/source/ob-bun/"
    :after (org)
    :config
    ;; Syntax highlighting only; Babel language activation happens once in `use-package org`.
    (add-to-list 'org-src-lang-modes '("bun" . typescript-ts))
    (add-to-list 'org-src-lang-modes '("bun" . typescript))
    (add-to-list 'org-src-lang-modes '("ts" . typescript-ts))
    (add-to-list 'org-babel-tangle-lang-exts '("bun" . "ts"))
    ;; Keep TypeScript Babel execution routed through ob-bun regardless of load order.
    (with-eval-after-load 'ob-typescript
      (defalias 'org-babel-execute:ts 'org-babel-execute:bun)
      (defalias 'org-babel-execute:typescript 'org-babel-execute:bun)
      (defalias 'org-babel-prep-session:ts 'org-babel-prep-session:bun)
      (defalias 'org-babel-prep-session:typescript 'org-babel-prep-session:bun)))
