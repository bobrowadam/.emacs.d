...
#+begin_src elisp
  ...
  (use-package ob-bun
    ...
    ;; Explicitly set Bun source blocks to use TypeScript syntax highlighting
    (with-eval-after-load 'org
      (add-to-list 'org-src-lang-modes '("bun" . typescript))
      
      (defun bob/setup-bun-syntax-highlight ()
        "Ensure Bun source blocks use TypeScript syntax highlighting."
        (font-lock-add-keywords 
         nil 
         '(("^[ \t]*#\\+begin_src[ \t]+bun" 0 'org-block-begin-line)
           ("^[ \t]*#\\+end_src" 0 'org-block-end-line))))
      
      (add-hook 'org-mode-hook #'bob/setup-bun-syntax-highlight)
      
      ;; Fallback syntax highlighting for Bun
      (setq org-src-lang-modes 
            (cons '("bun" . typescript) 
                  (assq-delete-all 'bun org-src-lang-modes)))
      
      ;; Ensure languages are registered on Org mode initialization
