...
#+begin_src elisp
  ...
  ;; `rewrite' preset with read-only tools, advice on
  ;; `gptel--suffix-rewrite' that applies the preset automatically,
  ;; and an output sanitiser that strips markdown fences /
  ;; surrounding blank lines.  See `modules/bob-gptel-rewrite.el'.
  (use-package bob-gptel-rewrite
    :ensure nil
    :after gptel
    :demand t
    :commands (bob/gptel-insert)
    :config
