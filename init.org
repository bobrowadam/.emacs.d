  ...
  (defun check-types-command ()
    ...
    (when-let* ((default-directory (locate-dominating-file default-directory "package.json"))
                ...
                (package-json (json-parse-string package-json-raw
                                                 :object-type 'alist)))
      (or (assocdr 'check-types (assocdr 'scripts package-json))
          (assocdr 'typecheck (assocdr 'scripts package-json))
          (assocdr 'type-check (assocdr 'scripts package-json)))))
  ...
  (defun npm-run-build ()
    ...
    (if-let* ((default-directory (locate-dominating-file default-directory "package.json"))
        ...
        (cond ((and (not (eq major-mode 'comint-mode))
              ...
              ((s-starts-with-p "bun" local-check-types-command)
               (compilation-start "bun run check-types"
                                  t (lambda (_)
                                      compilation-buffer-name)))
              (t
