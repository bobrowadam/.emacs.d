...
Local uses hardcoded =postgres/postgres= credentials. Remote dev/prod
connections ensure AWS SSO is logged in, fetch the database password from SSM,
start an SSM tunnel on =localhost:5432= via =task db-connect ENV=dev= (or
=ENV=prod=), and then connect.
