...
#+begin_src elisp
  (use-package gptel
    ...
    ;; Opus 4.7 was released 2026-04-16; remove this once gptel ships it.
    (if (memq 'claude-opus-4-7 (gptel-backend-models gptel-backend))
        (display-warning 'gptel "claude-opus-4-7 already in gptel models, remove manual addition from init.org")
      (push 'claude-opus-4-7 (gptel-backend-models gptel-backend)))
  ...
  (use-package bob-gptel-tools
    ...
    ;; Register the `pi' preset: Claude thinking + all pi tools + pi
    ;; system prompt.  This is the standard agentic-flow config that
    ;; new gptel buffers already inherit from global defaults.  Having
    ;; it as an explicit preset lets you restore it inside any buffer
    ;; (e.g. after the `rewrite' preset was applied) and keeps it out
    ;; of the rewrite flow.
