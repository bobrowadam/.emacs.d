...
Connection passwords are read from =~/.authinfo.gpg=. A connection is only
registered when authinfo has a matching =machine= line — so to add dev or
prod you just append a line and =M-x bob/ejc-refresh-connections=.
...
Remote (dev/prod) connections assume a local SSM tunnel on =localhost:5432=,
started by =task db-connect ENV=dev= (or =ENV=prod=) in the monorepo.
