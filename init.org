...
#+begin_src elisp
  ...
  Returns definition location, references, type info, and hover information."
    ...
    ;; Monkey patching: (maybe send a PR)
...
#+begin_src elisp
  (use-package magit
    ...
    (defun bob/setup-grain-project-for-worktree (path)
      ...
      (make-symbolic-link "~/dotfiles/.claude/"
                          (format "%s/.claude/" path))
      (make-symbolic-link "~/dotfiles/CLAUDE.md"
...
#+begin_src elisp
  (use-package flymake-eslint
    ...
    (jtsx-tsx-mode . flymake-eslint-enable))
#+end_src
...
#+begin_src elisp
  ...
  (ert-deftest generate-command ()
    (should (equal (bob/generate--run-service-command "mail-service" "development") "NODE_ENV=development TS_NODE_PROJECT='./apps/backend/mail-service/tsconfig.app.json' TS_NODE_FILES=true nodemon --exec \"node --inspect -r ts-node/register -r tsconfig-paths/register ./apps/backend/mail-service/src/index.ts\"")))
