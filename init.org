...
*** Open Pi in Kitty
#+begin_src elisp
  (global-set-key (kbd "C-c m P") #'bob/open-pi-in-kitty)

  (defvar bob/kitty-pi-tabs (make-hash-table :test 'equal)
    "Maps monorepo root dirs to Kitty tab IDs for pi sessions.")

  (defun bob/open-pi-in-kitty ()
    "Open or focus Pi coding agent in Kitty for the current monorepo root."
    (interactive)
    (let* ((dir (expand-file-name (bob/monorepo-root)))
