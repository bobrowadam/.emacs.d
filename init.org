...
#+begin_src elisp
  ...
  (use-package org
    ...
    (org-capture-templates
     `(("t" "entry" entry
        (file
         ,(concat org-directory "20240104T120451--inbox__project.org"))
        "* %?\n  %i")
  ...
  (use-package org-agenda
    ...
    (org-agenda-custom-commands
     `(("b" tags "+OngoingBugs")
       ...
       ("j" "Interstitial journal (today)" bob/journal-show-today)
       ("d" "Daily view"
    ...
    (defun bob/journal-show-today (&optional _)
      "Open the interstitial journal and narrow to today's datetree subtree."
      (interactive)
      (let ((file (concat org-directory "journal.org"))
            (today (decode-time)))
        (find-file file)
        (widen)
        (goto-char (point-min))
        (org-datetree-find-date-create
         (list (nth 4 today) (nth 3 today) (nth 5 today)))
...
#+begin_src elisp
  (use-package doom-themes
    ...
    :init
    (defun bob/register-and-load-themes (themes)
      "Register THEMES and load the first one in the list.
  Creates a `bob/cycle-themes' function to cycle through the themes."
      (unless themes
        (error "Themes list cannot be empty"))
      (let ((theme-list themes)
            (current-index 0))
        (load-theme (car theme-list) t)
        (defun bob/cycle-themes ()
          "Cycle through registered themes."
          (interactive)
          (setq current-index (mod (1+ current-index) (length theme-list)))
          (mapc #'disable-theme custom-enabled-themes)
          (load-theme (nth current-index theme-list) t))))
...
#+begin_src elisp
  (use-package ghostel
    ...
    (:map ghostel-mode-map
          ("C-c C-e" . ghostel-copy-mode))
    (:map ghostel-copy-mode-map
          ("C-c C-j" . ghostel-copy-mode-exit)))
