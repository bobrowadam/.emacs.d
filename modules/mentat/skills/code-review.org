#+TITLE: Present code review
#+DESCRIPTION: Present validated review comments through Bob’s Emacs review interface.
#+PROPERTY: header-args:emacs-lisp :lexical yes :tangle yes

* Present a review

Use =mentat-code-review-present= after review findings are complete. It changes only the local Emacs review interface; it does not submit a remote review. Preserve file paths and line numbers from inspected source. Enable speech only when Bob requests it.

* Native implementation

#+name: code-review-operations
