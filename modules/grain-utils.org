...
#+begin_src elisp
  (defun bob/setup-grain-project-for-worktree (path)
    ...
    (shell-command "npm run rules:sync -- --agent=claudemd")
    (shell-command "npm run rules:sync -- --agent=claudeskills")
...
*** Globals
#+begin_src elisp
  (defconst *grain-main-worktree-directory* (expand-file-name "~/source/grain"))
...
#+begin_src elisp
  ;;;###autoload
  (defun grain/check-waiting-in-staging ()
    "Check the commits that are currently waiting on staging"
    (interactive)
    (let ((default-directory *grain-main-worktree-directory*))
      (shell-command "./scripts/waiting-in-staging.sh")))
#+end_src
